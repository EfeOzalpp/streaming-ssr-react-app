...
return (
  <>
    ...
    <div
      ref={hostRef}
      className="coin"
      onClick={ctaEnabled ? onClick : undefined}
      style={{ pointerEvents: ctaEnabled ? 'auto' : 'none' }}
    />

    {/* Stable slot next to coin */}
    <div className="copy-slot">
      <h1
        ...
        style={{ pointerEvents: ctaEnabled ? 'auto' : 'none' }}
      >
        {ctaEnabled ? label : 'Loading…'}
      </h1>

      {/* ALWAYS present wrapper so layout never changes */}
      <div className="loading-hub-wrap" aria-hidden={ctaEnabled ? 'true' : undefined}>
        {/* Only the inside mounts/unmounts; wrapper stays */}
        {!ctaEnabled && (
          <LoadingHub
